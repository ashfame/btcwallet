...
func TestBIP32SpecTestVector(t *testing.T) {
	...
	for _, table := range tables {
		seed, err := hex.DecodeString(table.seed)
		if err != nil {
			t.Errorf("unable to decode seed: %s\n", err.Error())
		}
		...
		for _, node := range table.nodes {
			...
			if xpub != node.pubKey {
				t.Errorf("pub key did not match for path: %s\nExpected: %s\nGot: %s", node.path, node.pubKey, xpub)
			}
			if xprv != node.prvKey {
				t.Errorf("prv key did not match for path: %s.\nExpected: %s\nGot: %s", node.path, node.prvKey, xprv)
			}
		}
...
func TestBIP39SpecTrezorTestVector(t *testing.T) {
	tables := []struct {
		entropy          string
		mnemonic         string
		seed             string
	...
	for _, table := range tables {
		...
		if err != nil {
			t.Errorf("decoding entropy hex string failed")
		}
		mnemonic := vault.GenerateMnemonicByEntropy("English", entropy)
		if mnemonic != table.mnemonic {
			t.Errorf("generated mnemonic did not match:\nExpected: %s\nGot: %s", table.mnemonic, mnemonic)
		}

		err = vault.InitializeWallet(mnemonic, "TREZOR")
		if err != nil {
			t.Errorf("wallet could not be initialized out of mnemonic: %s", mnemonic)
		}
		seed, _ := vault.ExportSeed()
		if seed != table.seed {
			t.Errorf("seed did not match:\nExpected: %s\nGot: %s", table.seed, seed)
		}

		xprv, _, err := vault.GetNodeKeys("m")
		if err != nil {
			t.Errorf("error deriving master node for wallet")
		}

		if xprv != table.masterPrivateKey {
			t.Errorf("master node private key did not match:\nExpected: %s\nGot: %s", table.masterPrivateKey, xprv)
		}
	}
}

// func TestBIP39SpecJapaneseTestVector(t *testing.T) {
// 	tables := []struct {
// 		entropy          string
// 		mnemonic         string
// 		passphrase       string
...
// 			t.Errorf("decoding entropy hex string failed")
// 		}
// 		mnemonic := vault.GenerateMnemonicByEntropy("Japanese", entropy)
// 		if mnemonic != table.mnemonic {
// 			t.Errorf("generated mnemonic did not match:\nExpected: %s\nGot: %s", table.mnemonic, mnemonic)
// 		}

// 		err = vault.InitializeWallet(mnemonic, table.passphrase)
// 		if err != nil {
// 			t.Errorf("wallet could not be initialized out of mnemonic: %s", mnemonic)
// 		}
// 		seed, _ := vault.ExportSeed()
